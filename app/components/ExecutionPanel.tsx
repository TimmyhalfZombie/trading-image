...
import { BadgeCheck, TrendingUp, TrendingDown, Minus, Crosshair, Loader2 } from 'lucide-react';

/* ── Skeleton shimmer animation (injected once via <style>) ───────── */
...
/* ── Tiny reusable skeleton block ─────────────────────────────────── */
function Bone({ className = '', style = {} }: { className?: string; style?: React.CSSProperties }) {
    return <div className={`hive-skeleton-bone ${className}`} style={style} />;
}

/* ── Skeleton layout that mirrors the real result ─────────────────── */
function SkeletonLoading() {
    return (
        <div className="flex flex-col gap-5 animate-in fade-in duration-500">

            {/* ── Header row: "Signal" label + confidence badge ── */}
            <div className="flex flex-col gap-3 flex-shrink-0">
                <div className="flex items-center justify-between">
                    <Bone style={{ width: 48, height: 12 }} />
                    <Bone style={{ width: 120, height: 28, borderRadius: 9999 }} />
                </div>

                {/* ── Big signal hero box ── */}
                <div
                    className="flex items-center justify-center gap-3 py-7 rounded-xl"
                    style={{
                ...
                >
                    ...
                    <Bone style={{ width: 120, height: 36 }} />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 flex-shrink-0">
                ...
                >
                    <Bone style={{ width: 64, height: 10, marginBottom: 10 }} />
                    <Bone style={{ width: 80, height: 22 }} />
                </div>
                {/* Take Profit card */}
                <div
                    className="rounded-xl p-4"
                    style={{ backgroundColor: 'var(--surface-alt)', border: '1px solid var(--border-light)' }}
                >
                    <Bone style={{ width: 72, height: 10, marginBottom: 10 }} />
                    <Bone style={{ width: 80, height: 22 }} />
                </div>
            </div>
            ...
            <div
                ...
                style={{ backgroundColor: 'var(--surface-alt)', border: '1px solid var(--border-light)' }}
            >
                <Bone style={{ width: 80, height: 10, marginBottom: 16 }} />
                <div className="flex flex-col gap-2.5">
                    <Bone style={{ width: '100%', height: 12 }} />
                    <Bone style={{ width: '92%', height: 12 }} />
                    <Bone style={{ width: '85%', height: 12 }} />
...
export function ExecutionPanel({ status = 'awaiting', result }: ExecutionPanelProps) {
    ...
    return (
        <>
            {/* Inject skeleton keyframe styles once */}
            <style dangerouslySetInnerHTML={{ __html: skeletonStyles }} />

            <div
                className="panel-container flex flex-col h-full rounded-2xl shadow-sm overflow-hidden transition-colors duration-300"
                style={{
            ...
            >
                ...
                >
                    ...
                    {isAnalyzing ? (
                        <Bone style={{ width: 72, height: 24, borderRadius: 8 }} />
                    ) : result?.asset ? (
                        <span
